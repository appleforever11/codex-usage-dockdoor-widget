        ...
        Button(action: action) {
            ...
            .background {
                GeometryReader { geometry in
                    CodexIdentityArtwork(identity: identity, isEmphasized: isSelected || isHovering)
            ...
            .overlay {
                RoundedRectangle(cornerRadius: 11, style: .continuous)
                    .strokeBorder(
                        LinearGradient(colors: isSelected
                            ? [.white.opacity(0.92), identity.accent, identity.accent.opacity(0.65)]
                            : [.white.opacity(isHovering ? 0.45 : 0.20), identity.accent.opacity(0.16)],
            ...
            .overlay(alignment: .topLeading) {
                if isSelected {
                    Image(systemName: "checkmark")
                        ...
                        .foregroundStyle(.white)
                        .padding(3)
                        .background(identity.accent.opacity(0.5), in: Circle())
                        .padding(5)
                }
            }
            .shadow(color: identity.accent.opacity(isSelected ? 0.30 * tuning.clampedGlow : 0), radius: 6, y: 1)
...
private struct CodexIdentityArtwork: View {
    let identity: CodexTheme
    ...
    var body: some View {
        TimelineView(.animation(minimumInterval: 1.0 / 18.0,
                                paused: reduceMotion || !tuning.animationsEnabled || !isVisible)) { timeline in
            ...
            Canvas { context, size in
                ...
                glow(&context, at: center, radius: size.height * 0.95,
                     color: identity.accent.opacity((isEmphasized ? 0.55 : 0.35) * tuning.clampedGlow))
                switch identity {
                case .astra:
                    glow(&context, at: CGPoint(x: size.width * 0.34, y: size.height * 0.68),
                         radius: size.width * 0.65, color: .purple.opacity(0.7 * tuning.clampedGlow))
